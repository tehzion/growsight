...
const Sidebar = ({ isOpen, toggleSidebar, user }: SidebarProps) => {
  ...
  const navItems = [
    ...
    { path: '/users', label: 'Users', icon: <Users className="h-5 w-5" />, roles: ['super_admin', 'org_admin', 'subscriber'], permission: 'users.view' },
    { path: '/assessments', label: 'Assessments', icon: <ClipboardList className="h-5 w-5" />, roles: ['super_admin', 'org_admin', 'subscriber'], permission: 'assessments.create' },
    ...
    { path: '/email-service', label: 'Email Service', icon: <Settings className="h-5 w-5" />, roles: ['super_admin'] },
    { path: '/reporting', label: 'Reporting', icon: <BarChart3 className="h-5 w-5" />, roles: ['super_admin', 'org_admin'] },
    { path: '/assessment-assignments', label: 'Assignments', icon: <UserCheck className="h-5 w-5" />, roles: ['org_admin', 'reviewer'], permission: 'assessments.assign' },
    { path: '/results', label: 'Analytics', icon: <BarChart4 className="h-5 w-5" />, roles: ['super_admin', 'org_admin'], permission: 'analytics.org' },
    { path: '/assessment-results', label: 'Assessment Results', icon: <Activity className="h-5 w-5" />, roles: ['super_admin', 'org_admin'], permission: 'analytics.org' },
    { path: '/assessment-360', label: '360° Assessments', icon: <Users className="h-5 w-5" />, roles: ['super_admin', 'org_admin'], permission: 'analytics.org' },
    { path: '/import-export', label: 'Import/Export', icon: <Database className="h-5 w-5" />, roles: ['super_admin', 'org_admin'] },
    { path: '/teams', label: 'Team Management', icon: <Users className="h-5 w-5" />, roles: ['super_admin', 'org_admin'], permission: 'teams.manage' },
    { path: '/my-results', label: 'My Results', icon: <BarChart4 className="h-5 w-5" />, roles: ['subscriber', 'employee', 'reviewer'] },

    { path: '/subscriber-assessments', label: 'My Assessments', icon: <ClipboardList className="h-5 w-5" />, roles: ['subscriber'] },
    { path: '/competencies', label: 'Competencies', icon: <Tag className="h-5 w-5" />, roles: ['super_admin', 'org_admin'], permission: 'assessments.create' },
    ...
    { path: '/my-goals', label: 'My Goals', icon: <Target className="h-5 w-5" />, roles: ['employee', 'reviewer', 'subscriber'] },
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div
        className={`fixed inset-y-0 left-0 z-30 w-64 bg-primary-800 text-white transform transition duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-auto lg:h-screen ${isOpen ? 'translate-x-0' : '-translate-x-full'
          }`}
      >
