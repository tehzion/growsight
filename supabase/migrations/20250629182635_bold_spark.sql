/*
  # Create competency framework tables and functions
  ...
  3. Demo Data
    - Add sample competencies for demonstration
*/
...
DECLARE
  ...
  org3_id uuid;
BEGIN
  -- Get the UUIDs of the demo organizations
  SELECT id INTO org1_id FROM organizations WHERE name = 'Acme Corporation' LIMIT 1;
  SELECT id INTO org2_id FROM organizations WHERE name = 'TechStart Solutions' LIMIT 1;
  SELECT id INTO org3_id FROM organizations WHERE name = 'Global Enterprises' LIMIT 1;
  
  -- Only insert if we found the organizations
  IF org1_id IS NOT NULL THEN
    INSERT INTO competencies (name, description, organization_id)
    VALUES 
      ('Leadership', 'Ability to lead and inspire teams', org1_id),
      ('Communication', 'Effective verbal and written communication skills', org1_id),
      ('Problem Solving', 'Ability to analyze and solve complex problems', org1_id),
      ('Teamwork', 'Ability to collaborate effectively with others', org1_id),
      ('Technical Skills', 'Proficiency in required technical areas', org1_id)
  ...
  IF org2_id IS NOT NULL THEN
    INSERT INTO competencies (name, description, organization_id)
    VALUES 
      ('Innovation', 'Ability to generate new ideas and approaches', org2_id),
      ('Customer Focus', 'Understanding and meeting customer needs', org2_id)
    ON CONFLICT (organization_id, name) DO NOTHING;
  END IF;
  ...
  IF org3_id IS NOT NULL THEN
    INSERT INTO competencies (name, description, organization_id)
    VALUES 
      ('Strategic Thinking', 'Ability to think and plan for the long term', org3_id)
    ON CONFLICT (organization_id, name) DO NOTHING;
  END IF;
END
$$;